...
router.post('/create', usersOnly, body('title')
	...
	async (req, res) => {
		...
		if (errors.isEmpty()) {
			...
			await req.dbServices.custom.create(play)
			res.redirect('/')
		} else {
			const errorContext = {
				title: 'Create',
...
router.get('/like/:id', usersOnly, async (req, res) => {
	await req.dbServices.custom.likeById(req.params.id, req.user._id)

	res.redirect('/')
})

router.get('/sort-by-date', async (req, res) => {
	const plays = await req.dbServices.custom.getFirstNByDateDesc(4)
...
router.get('/sort-by-likes', async (req, res) => {
	const plays = await req.dbServices.custom.getFirstNbyLikesDesc(4)
